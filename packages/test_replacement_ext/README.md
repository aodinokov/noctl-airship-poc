...
The next file we want to check is the configuration of replacement transformer that will set values from the catalogue in the original secret. It has the same name as the file it modifies, but is located in the replacement directory in [replacements/secret.yaml](manifests/function/ephemeral/replacements/secret.yaml).
...
That in reality will mean 3 resources: secret and catalogue from `ephemeral function` and catalogue from `gating type`.
...
Don't forget to add [replacements/catalogue.yaml](manifests/type/gating/replacements/catalogue.yaml) that will remove the catalogue after using it. We also mention both transformers configuration in [replacements/kustomization.yaml](manifests/type/gating/replacements/kustomization.yaml):
...
Please pay attention to the order of resources. Do you remember that our resources consist of 3 resources: secret and catalogue from `ephemeral function` and catalogue from `gating type`? If this 'replacements/kustomization.yaml' is called at a transformer it will perform modification of catalogue in `ephemeral function` based on the data from the catalog in `gating type`, after that will execute all transformers from `ephemeral function`, that in our case will modify the `secret` fields and delete the catalogue from `ephemeral function` and the last line here deletes the catalogue from `gating type`. Basically, this approach allows us to *incapsulate* `ephemeral function` into `gating type` in the way so the upper level may know about/modify the catalogue only for `gating type`.

We covered all used files on the `gating type` and now may go to the [kustomization.yaml](manifests/site/ephemeral/bootstrap/kustomization.yaml) on site level.
...
This script will download the required kustomize versoin (minimal requirement is 3.8.1) and execute the described configuration.
...
*Note:* if you don't have a place to run this, please refer to [expected_output.yaml](expected_output.yaml) and [expected_changes.diff](expected_changes.diff) where I copied the values I've got after run.

There are 2 possible ways to modify parameters on site level:
 * using the same approach with replacements
 * using strategic merge patch (SMP) for catalogues
 
 If the type layer already doesn't have any duplicated parameters, there is no a big need in the replacement approach, since changing one of the parameters already has to apply changes to all dependend fields. The site-layer in most cases works only with specific values that's why the approach with SMP for the type catalogue will be simpler and quicker.
